...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_insert = "INSERT INTO sinh_vien (LopBC, HoTen, NgaySinh, DiaChi, MaKhoa) VALUES (?, ?, ?, ?, ?)";
    $stmt_insert = $connec->prepare($sql_insert);
    $stmt_insert->bind_param("ssssi", $LopBC, $HoTen, $NgaySinh, $DiaChi, $MaKhoa);
        ...
        <div class="form-group">
            ...
            <select name="MaKhoa" required>
                ...
                <?php while ($row = $khoa_result->fetch_assoc()) : ?>
                    <option value="<?php echo $row['MaKhoa']; ?>"><?php echo htmlspecialchars($row['TenKhoa']); ?></option>
                <?php endwhile; ?>
            </select>
        </div>
        <div class="form-group">
